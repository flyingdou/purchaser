 ...
 * @returns
 */
function createSelector(_obj) {
	if($("#iosSelector").length == 0){
		$("body").append("<input type='hidden' id='iosSelector' value=''>");
	}
	var index = $("#iosSelector").val();
	var bankSelect = new IosSelect(
		1, 
        [_obj.data],
        {
            container: '.container',
            title: _obj.title ? _obj.title : '选择',
            itemHeight: 50,
            itemShowCount: 3,
            oneLevelId: index,
            showAnimate: true,
 ...
 * @returns
 */
function createCitySelector(func){
	if($("city").length == 0){
	...
	var iosSelect = new IosSelect(2, 
            [iosProvinces, iosCitys, iosCountys],
            {
                ...
                callback: function (selectOneObj, selectTwoObj, selectThreeObj) {
                    ...
                    $("city").attr("data-province", selectOneObj.value);
                    $("city").attr("data-city", selectTwoObj.value);
                    $("city").attr("data-district", selectThreeObj.value);
                    func(selectOneObj, selectTwoObj, selectThreeObj);
                }
        });
}
    ...
    var iosSelect = new IosSelect(3, 
        [yearData, monthData, dateData],
        {
            ...
            callback: function (selectOneObj, selectTwoObj, selectThreeObj) {
                ...
                // 回调传回用户选择的值
                func(selectOneObj.id, selectTwoObj.id, selectThreeObj.id);
            }
    });
}